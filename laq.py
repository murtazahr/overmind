...
Ncr=np.zeros(Iter)
Nbitr=np.zeros(Iter)  # SL: number of bits. not sure how it's counted
...
gr=np.zeros((M,nv))  # SL: gradients per clients?
mgr=np.zeros((M,nv))
dL=np.zeros((M,nv))
...
for k in range(0,Iter):
    ...
    for m in range(0,M):
        ...
        for (batch, (images,labels)) in enumerate(Datatr[m].take(1)):
            ...
            gr[m,:]=vec
            dvec = vec - bvec  # SL: dvec is the difference of the current gradients and the one from previous batch
            e[m] = (dvec.dot(dvec))  # SL: epsilon in the paper. (quantization error)
        ...
        dL[m,:]=gr[m,:]-mgr[m,:]  # SL: the left term of equ. (7a) from the paper
        if ((dL[m,:].dot(dL[m,:]))>=(1/(alpha**2*M**2))*me[m]+ec*3*(e[m]+ehat[m])+ade or clock[m]==C):    # SL: equation (7a) from the paper
            Ind[m,k]=1
